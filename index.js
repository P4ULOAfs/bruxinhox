...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FadhilGraphy = ["19034375199@s.whatsapp.net"]
			...
			mess = {
				wait: '*[ WAIT ]* ⏳AGUARDE UM POUCO MANO...',
				success: '*[💯] Sucesso...*',
				error: {
				...
				only: {
					...
					ownerB: '*[⚠️]Desculpe, este comando só pode ser usado pelo proprietário do barco!* ',
			...
			const ownerNumber = [
			"19034375199@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"19034375199@s.whatsapp.net","6283102650464@s.whatsapp.net"//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
		...
		case 'daftar':
			...
			case 'fadhil':
                ...
                setTimeout( () => {
					...
					setTimeout( () => {*/
					client.sendMessage(from, 'Obrigado por usar o*ARR157-BOT* \n\nSe inscrevam no meu canal fml🔥😏 https://youtube.com/channel/UCalQOjX5BFqim6x8JyCMM-w', text, {quoted: mek})
					}, 0)
    				break
/*********PISAH MENU**********/
              case 'ownermenu':
...
*/
              case 'anime':
                ...
                if (!isUser) return reply(mess.only.userB)
                if (isLimit(sender)) return reply(limits.limitend(pushname2))
                if (!isAnime) return reply(' *Anime Deve ativar o modo Anime* ')
					anp = getRandom('.gif')
					rano = getRandom('.webp')
	   ...
	   case 'coli':
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					reply(`\`\`\`deleta todo o chat ARR157-BOT\`\`\``)
					break
                                case 'bcgc':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply(' A marque o fdp que você quer chutar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
				...
				case 'toimg':
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					if (!isQuotedSticker) return reply(' responder stickernya gan')
...
// Akhir Fitur Premium 
		...
		if (isLimit(sender)) return reply(limits.limitend(pushname2))
		 ...
		 case 'tomp3':
				...
				if (!isUser) return reply(mess.only.userB)
                	client.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('_*Responder vídeo mano!*_')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
